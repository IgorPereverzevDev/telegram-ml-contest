...
fn main() {
    ...
    let mut reader = ReaderBuilder::new().has_headers(true).delimiter(b',').from_reader(file);
    let df: Array2<String> = reader.deserialize_array2((100,2)).unwrap().into_shape((2, 100)).unwrap();

    print!("{:?}", df.index_axis(ndarray::Axis(0), 0));
...
//        .unwrap();
    ...
    // let labels_vector: Array2<u64> = reader.deserialize_array2((100, 2)).unwrap();
